...
- ✅ **Throughput Phase 2 (3/week extra-post cap) DEPLOYED TO PROD 2026-07-11** (`a279244`, migration `20260711140000_weekly_extra_post_cap` verified on both envs): `PlatformSettings.weeklyExtraPostCap` (default 3, admin-tunable), quota = account-scoped drafts in rolling 7d (dashboard ad-hoc posts are the only draft-creating path — cadence writes posts directly), 429 + reset date on `POST /api/ai/generate` (admins exempt), `GET /extra-post-quota` + dashboard remaining-count hint/toast
- ✅ **Throughput Phase 3 + 3b (prompt-drift guard + remediation) DONE 2026-07-11**: `packages/db/scripts/diff-prompt-rows.ts` (read-only; fingerprint per env + diff mode; exits 1 on drift; header documents the in-container snippet for prod). Remediated: prod 202 overwritten with staging's live-verified carousel-plan text (prod's had URL-first caption instructions — the root cause of the E2E's caption-shaped-JSON bug); prod 32 synced to staging's "Click here >>" CTA (user decision); 201 refreshed on BOTH envs to in-code seed defaults (anthropic/claude-sonnet-4-5, was stale openai/gpt-4o-mini everywhere). Verified end state: 78 rows in sync; only 218 differs (cosmetic, auto-resolves at Phase 1e) + 7 intentionally-parked staging rows (below)
- 🧾 **RELEASE CHECKLIST — before every prod deploy**: (1) prod in-flight check (runs/jobs/due posts); (2) prompt-row drift check — fingerprint staging (`pnpm --filter @socioply/db exec tsx scripts/diff-prompt-rows.ts --out staging.json`) + prod (in-container node snippet in the script header), then `--diff staging.json prod.json --labels staging,prod`; exit 1 = drift → resolve deliberately (admin edits are per-environment BY DESIGN; anything edited on staging that should ship must be replayed to prod explicitly). As of 2026-07-11 the report is fully clean (0 differing, 0 env-only) — any future hit is real drift
...
- 📋 **Billing + multi-tenancy hardening — PLANNED 2026-07-12, see `.plans/multi-tenancy-hardening.implementation-plan.md`**: billing runs through GHL (whitelabel SaaS mode; workflow Subscription triggers → webhook receiver — NOT Stripe-direct as originally assumed), account state machine (active/paused/cancelled + paidThrough; "paidThrough governs publishing, status governs generation"), re-anchor `subscriptionStartedAt` on every payment (reactivation = just a payment), burst-on-payment-cleared, 60d pause→cancel + 90d cancel→delete clocks, manual+auto deletion path (LLMUsage detached, never deleted), social cost logging, failure-alert gap closure, account-scope audit, deploy in-flight gate + staging paths filter. Deferred by user: B4 DB upsize (pre-launch tripwire), per-vertical packs (first batch = chiros), automated account-health checks (manual onboarding covers concierge batch)
- ✅ **GHL-embedded chat onboarding IMPLEMENTED 2026-07-13** (staging `a3eb5b9`..`ac83835`, NOT yet on prod — see `.plans/ghl-onboarding.implementation-plan.md`): Phase 0 marketplace-app shell + SSO-as-session (synthetic `ghl:<userId>` clerkId, `emb_` bearer tokens, frame-ancestors CSP on /embed only, Clerk untouched on open web); Phase 1 scripted 17-step resumable chat engine + UI; Phase 2 GHL Business-Profile prefill + background website crawl + Gemini-vision semantic palette + specialization detection; Phase 3 voice answers (MediaRecorder → S3 archive ALWAYS → Gemini transcription → editable transcript); Phases 4–7 brand-profile synthesis, writing voice (labeled transcripts + article), template reveal writing all nl* fields, hemisphere-aware AHPRA-safe offer calendar → NewsletterOffer rows, CTA drafts, WordPress connect w/ live verify + explicit opt-out, Social Planner account pickup, ElevenLabs branch (key + voice clone from chat audio), toggles, **Generation-Readiness Validator** (19-row canonical check; completion impossible while red; generation gated on `onboardingCompletedAt` for session-holding accounts only), finale = calendars routed + first burst fired. Confirm-card UI polish complete (business form, logo picker, profile editor, live srcdoc template reveal with swatches, offers editor, WP walkthrough form, EL guided key entry). Install-flow OAuth stub at /api/embed/oauth-callback
- 🧪 **NEXT: onboarding Phase 8 E2E — full guide + checklist in `.documentation/onboarding-testing-guide.md`** (marketplace app build steps incl. minimal scopes + SSO key, env wiring `GHL_SSO_SECRET`/`NEXT_PUBLIC_API_URL`/`EMBED_FRAME_ANCESTORS`, test-account provisioning w/ Private-Integration scopes, 15-step walkthrough table with server-side verification per step, known risks: iframe microphone permission, crawl quality on the real site; prod rollout steps). GHL_SSO_SECRET currently UNSET on both envs — session exchange 503s until set
- ✅ **Non-ElevenLabs video→carousel conversion IMPLEMENTED 2026-07-13** (`f306e21`): no-voice accounts (toggle+key+voiceId absent) get accent-tinted `brand_tint_accent` carousels in place of ALL video slots via matrix-level `applyVoiceCapability` substitution; companion pitch-hook stories auto-swap to pitch-carousel; accent = onboarding-approved `nlLinkColor`; per-run self-healing both directions; Wed/Sat primary tints + encode-failure fallback untouched; no-voice accounts fully immune to the ffmpeg-encode concern
- 📋 **GHL-embedded chat onboarding — original plan reference, see `.plans/ghl-onboarding.implementation-plan.md`**: private marketplace app (sidebar Custom Page, iframe, GHL SSO as session — no Clerk in the iframe), scripted chat flow ending with 100% generation-ready accounts (Generation-Readiness Validator = definition of done); GHL Business Profile prefill (location API schema live-verified incl. `social{}` object), website crawl + vision-LLM palette (semantic roles → newsletter/email templates built + confirmed via live preview reveal), 5 Manifesto-derived questions answered by voice (audio → ElevenLabs corpus + transcripts → writing voice w/ real article blend), offer calendar + social CTAs + WordPress connection + Social Planner linking + ElevenLabs branch; payment-before-onboarding with `onboardingCompletedAt` gating first generation; finale = "start generating now" button. Follow-up noted: hook-video→carousel flip for non-ElevenLabs accounts
- 📋 **Non-ElevenLabs video→carousel conversion — PLANNED 2026-07-13, see `.plans/non-elevenlabs-carousel-conversion.implementation-plan.md`**: accounts without a working EL voice get NO video slots — hook videos, quote videos AND reels become brand_tint carousels with the ACCENT color as the full-page overlay (matrix-level substitution incl. companion-story swap, per-run self-healing both directions; music slideshows explicitly rejected — APIs can't put music on image carousels)
- 📋 **Lead-gen documents via Google Drive — PLANNED 2026-07-14, see `.plans/leadgen-drive-documents.implementation-plan.md`**: central service-account Drive (folder per account), prospects use Drive's Request Access → Access Proposals API (list/resolve — verified real) auto-approves + captures requester email as GHL contact with per-document tags; template library = designed HTML masters with brand tokens + constrained voice-rewrite (fact-locked, AHPRA-safe, dash-sanitized) → Chromium PDF, REVIEW-GATED; custom uploads = stamp-only cover page; starter library auto-compiled at onboarding completion. Build order starts with the 1h spike: access proposals on service-account-owned files
- 📋 **GHL snapshot spec FINALIZED 2026-07-15 — `.documentation/ghl-snapshot-guide.md`**: keep (Social Planner, email campaigns, contacts/tags, workflows, Conversations, Reputation, one New Patient pipeline), hide/off (Content AI!, Sites/funnels, client-facing Payments, memberships, marketplace access), defer SMS per-client; prebuild = 3 billing workflows w/ custom-value token trick, tag taxonomy (service vs marketing separation), leadgen nurture skeletons, dormant review-request workflow, hidden booking calendar. Booking = clinic's PMS via new bookingUrl field (international strategy); PMS dropdown = data capture; connector framework PARKED (.plans/pms-connector-framework...md, Cliniko presumptive first); review velocity at launch = newsletter ask-blocks + branded QR review counter card (added to leadgen starter library)
...
- ✅ **Kids-snack / tech-free "module" question RESOLVED 2026-07-11 — no build needed**: git history (commit `0d81182`, 2026-06-17 Phase 2a genericization) shows the modules were deliberately RETIRED, not half-built — `kids_snack` was consolidated into `recipe_2` (generic recipe chain) and `tech_free_activity` into `secondary_article` (generic article chain). Content plans carry kids-snack ideas in `recipe_2` and tech-free activities in `secondary_article`. Cleanup done: 7 leftover staging prompt rows deleted (retirement said "deactivate" but never touched the DB rows), stale csv.ts/schema comments fixed, dead admin badge code replaced with a live `recipe-2` badge
