...
- 📋 **Finish the kids-snack / tech-free newsletter modules** (user decision 2026-07-11): the scaffolding exists — calendar `modules` JSON slots (`kidsSnack`, `techFreeActivity`), admin calendar badges, CSV columns, and 7 seeded staging prompt rows (`nl_kids_snack_*` ×4, `nl_tech_free_*` ×3) — but `buildModules` in `apps/api/src/newsletter/generate.ts` only produces `recipe`/`recipe2` and nothing loads those prompt keys. Wire up research + writer + render (mirror the recipe module pattern), then seed the 7 rows on prod. Fits the `family_care` specialization
