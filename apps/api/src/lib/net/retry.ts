...
/** Duck-types provider error shapes (Fal ApiError, fetch Response-derived errors) that expose `status`. */
...
export function isRetryableNetworkError(err: unknown): boolean {
  ...
  return /ECONNRESET|ETIMEDOUT|ECONNREFUSED|EAI_AGAIN|fetch failed|network|socket hang up/i.test(message)
}
