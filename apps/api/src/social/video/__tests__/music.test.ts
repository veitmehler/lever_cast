...
describe('buildMusicFilterChain', () => {
  ...
  it('ducks −18dB (×0.1259) with a 0.5s ramp at the narration onset', () => {
    const chain = buildMusicFilterChain({ videoDuration: 148.3, duckAtSec: 5.042 })
    // 1 − 10^(−18/20) ≈ 0.8741
    expect(chain).toContain(`volume='1-0.8741*clip((t-5.042)/0.5\\,0\\,1)':eval=frame`)
    expect(chain).toContain('atrim=0:148.300')
    expect(chain).toContain('afade=t=out:st=146.300:d=2')
  })

  it('ducks from t=0 for narrated types without a title slide (S3)', () => {
    const chain = buildMusicFilterChain({ videoDuration: 29.7, duckAtSec: 0 })
    expect(chain).toContain(`clip((t-0.000)/0.5`)
  })
  ...
  it('respects custom duck depth, ramp, and fade-out', () => {
    ...
    // 1 − 10^(−12/20) ≈ 0.7488
    expect(chain).toContain('1-0.7488*clip((t-10.000)/1')
