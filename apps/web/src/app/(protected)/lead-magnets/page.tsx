...
/**
 * Lead Magnets library (leadgen plan Phase 5) — review-gated documents,
 * per-document tags, capture counts, Drive links.
 */
...
export default function LeadMagnetsPage() {
  ...
  const load = useCallback(async () => {
    const [d, t] = await Promise.all([fetch('/api/leadgen/documents'), fetch('/api/leadgen/templates')])
  ...
  async function act(id: string, action: 'approve' | 'regenerate') {
    setBusyId(id)
    try {
      const res = await fetch(`/api/leadgen/documents/${id}/${action}`, { method: 'POST' })
      if (!res.ok) {
  ...
  async function addFromTemplate(templateId: string) {
    const res = await fetch('/api/leadgen/documents/from-template', {
      method: 'POST',
  ...
  async function saveTags(id: string, raw: string) {
    ...
    const res = await fetch(`/api/leadgen/documents/${id}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
            ...
            <div className="flex items-start justify-between gap-3">
              ...
              <div className="flex shrink-0 gap-2">
                ...
                {(d.status === 'pending_review' || d.status === 'failed' || d.status === 'live') && d.kind === 'template' && (
                  <button
                    onClick={() => void act(d.id, 'regenerate')}
                    disabled={busyId === d.id}
