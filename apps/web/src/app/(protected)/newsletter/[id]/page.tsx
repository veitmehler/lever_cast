...
export default function NewsletterEditionPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function load() {
    ...
    setError(null)
    try {
      const res = await fetch(`/api/newsletters/${id}`, { cache: 'no-store' })
      if (!res.ok) {
  ...
  async function regenerate(section: string) {
    ...
    setError(null)
    try {
      const res = await fetch(`/api/newsletters/${id}/regenerate`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  async function approve() {
    ...
    setError(null)
    try {
      const res = await fetch(`/api/newsletters/${id}/approve`, { method: 'POST' })
      const data = await res.json().catch(() => ({}))
      if (!res.ok) {
        setError(data.error ?? `HTTP ${res.status}`)
        return
