...
export default function PlanPage() {
  ...
  async function generateSelected() {
    ...
    try {
      ...
      if (!res.ok) {
        toast.error(body.error ?? 'Failed to start generation')
        return
      }
      ...
      return (
        <div className="flex items-center gap-1.5">
          <input
            ...
            onKeyDown={(e) => e.key === 'Enter' && saveEdit(p.topicId!)}
            autoFocus
            className="flex-1 rounded-md border border-input bg-background px-2 py-1 text-sm"
          />
          <button onClick={() => saveEdit(p.topicId!)} className="rounded p-1 text-green-600 hover:bg-muted">
            <Check className="h-4 w-4" />
          </button>
          <button onClick={() => setEditKey(null)} className="rounded p-1 text-muted-foreground hover:bg-muted">
            <X className="h-4 w-4" />
          </button>
        </div>
      )
    ...
    if (!p) {
      return (
        <div className="flex items-center gap-2 text-muted-foreground">
          <button
            onClick={() => {
              setPickerDate(d.date)
              loadIdeas()
            }}
            className="inline-flex items-center gap-1 rounded-md border border-dashed border-border px-2 py-1 text-xs hover:bg-muted"
          >
      ...
      <div className="group flex items-start gap-2">
        ...
        <div className="flex items-center gap-0.5 opacity-0 transition-opacity group-hover:opacity-100">
          {p.source === 'article_calendar' && p.calendarTopicId ? (
            <button
              onClick={() => planTopic(d.date, p.topic, 'article_calendar')}
              disabled={busyDate === d.date}
              className="rounded p-1 text-xs text-primary hover:bg-muted"
              title="Use this suggestion (adds it to your plan, editable)"
            >
              Use
            </button>
          ) : p.topicId ? (
    ...
    <div className="mx-auto max-w-6xl">
      <div className="mb-6 flex items-start justify-between gap-4">
        ...
        <div className="flex items-center gap-1 rounded-lg border border-border bg-card p-1">
          <button
            ...
            className={`inline-flex items-center gap-1.5 rounded-md px-3 py-1.5 text-sm ${view === 'table' ? 'bg-muted font-medium text-foreground' : 'text-muted-foreground'}`}
          >
            <TableIcon className="h-4 w-4" /> Table
          </button>
          <button
            onClick={() => setView('grid')}
            className={`inline-flex items-center gap-1.5 rounded-md px-3 py-1.5 text-sm ${view === 'grid' ? 'bg-muted font-medium text-foreground' : 'text-muted-foreground'}`}
          >
            ...
            <thead className="border-b border-border text-xs text-muted-foreground">
              <tr>
                <th className="w-10 px-3 py-2">
                  <input
                    type="checkbox"
                    checked={selected.size > 0 && selected.size === selectableDates.length}
                    onChange={(e) => setSelected(e.target.checked ? new Set(selectableDates) : new Set())}
                    className="h-4 w-4 rounded border-input"
                  />
                </th>
                <th className="px-3 py-2">Date</th>
                <th className="px-3 py-2">
                  <span className="inline-flex items-center gap-1"><FileText className="h-3.5 w-3.5" /> Article</span>
                </th>
                <th className="px-3 py-2">
                  <span className="inline-flex items-center gap-1"><Mail className="h-3.5 w-3.5" /> Newsletter</span>
                </th>
              </tr>
            </thead>
            <tbody>
                ...
                return (
                  <tr key={d.date} className="border-t border-border align-top">
                    ...
                    <td className="px-3 py-2.5">
                      {d.newsletter ? (
                        <div>
                          <div className="text-sm text-foreground">{d.newsletter.topic}</div>
                          {d.newsletter.status && (
                            <span className="mt-0.5 inline-block rounded-full bg-muted px-1.5 py-0.5 text-[11px] text-muted-foreground">
                              {d.newsletter.status}
                            </span>
                          )}
                        </div>
          ...
          {data.days.map((d) => (
            <div key={d.date} className="rounded-xl border border-border bg-card p-3">
              <div className="mb-2 flex items-center justify-between">
                ...
                {(d.article.primary || d.newsletter) && (
                  <input
                    type="checkbox"
                    checked={selected.has(d.date)}
                    onChange={() => toggleSelect(d.date)}
      ...
      {pickerDate && (
        <Modal title={`Use an idea for ${fmt(pickerDate)}`} onClose={() => setPickerDate(null)}>
          {ideas.length === 0 ? (
            <p className="text-sm text-muted-foreground">No ideas in your bank yet. Capture some on the dashboard.</p>
          ) : (
            <div className="max-h-80 space-y-1 overflow-y-auto">
              {ideas.map((idea) => (
                <button
                  key={idea.id}
                  onClick={() => assignIdea(pickerDate, idea)}
                  disabled={busyDate === pickerDate}
        ...
        <Modal title={`Add an article topic for ${fmt(addDate)}`} onClose={() => setAddDate(null)}>
          <div className="flex flex-col gap-3">
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === 'Enter' && addText.trim()) {
                  planTopic(addDate, addText.trim(), 'manual')
                  setAddDate(null)
                }
              }}
              autoFocus
            ...
            <div className="flex justify-end gap-2">
              <Button variant="outline" onClick={() => setAddDate(null)}>
                Cancel
              </Button>
              <Button
                onClick={() => {
