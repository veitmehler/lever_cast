...
export default function AdminArticleCalendarDetailPage({
  params,
}: {
  ...
  async function load() {
    ...
    setError(null)
    try {
      const res = await fetch(`/api/admin/article-calendars/${id}`, { cache: 'no-store' })
      if (!res.ok) {
        setError(`HTTP ${res.status}: ${(await res.text()) || res.statusText}`)
        return
      }
      setCalendar((await res.json()).calendar)
    } catch (err) {
  ...
  async function postCsv(commit: boolean) {
    ...
    try {
      ...
      const res = await fetch(
        `/api/admin/article-calendars/${id}/csv${commit ? '?commit=true' : ''}`,
  ...
  return (
    <div className="max-w-5xl">
      ...
      <div className="mb-6 flex items-start justify-between">
        ...
        <span className="inline-flex items-center gap-2 rounded-md border border-border bg-card px-4 py-2 text-sm text-muted-foreground">
          <Users className="h-4 w-4" />
          {calendar._count.accounts} accounts auto-routed
        </span>
      </div>
      ...
      <div className="mb-8 rounded-xl border border-border bg-card p-5">
        ...
        <p className="mb-3 text-xs text-muted-foreground">
          Required columns: <code>date, topic</code>. Optional:{' '}
          <code>angle, keywords, outline_framework, category</code>. Re-uploading a date
          overwrites it (idempotent).{' '}
          <a
            href="/article-topics-template.csv"
            download
                ...
                <table className="w-full text-left text-xs">
                  <thead className="text-muted-foreground">
                    <tr>
                      ...
                      <th className="py-1 pr-3">Extras</th>
                    </tr>
                  </thead>
                  <tbody>
                    {dryRun.preview.slice(0, 20).map((r) => (
                      <tr key={r.rowNumber} className="border-t border-border">
                        ...
                        <td className="py-1 pr-3 text-muted-foreground">
                          {[
                            r.angle && 'angle',
                            r.keywords.length > 0 && `${r.keywords.length} kw`,
                            r.outlineFrameworkNumber != null && `framework ${r.outlineFrameworkNumber}`,
                            r.category && 'category',
                          ]
                            .filter(Boolean)
                            .join(', ') || '—'}
            ...
            <tbody>
              {calendar.topics.map((t) => (
                <tr key={t.id} className="border-t border-border">
                  <td className="whitespace-nowrap px-4 py-2 font-mono text-xs">{t.date.slice(0, 10)}</td>
                  <td className="px-4 py-2">{t.topic}</td>
                  <td className="px-4 py-2 text-xs text-muted-foreground">
                    {t.keywords.length > 0 ? t.keywords.join(', ') : '—'}
                  </td>
                  <td className="px-4 py-2 text-xs text-muted-foreground">
                    {t.outlineFrameworkNumber ?? '—'}
                  </td>
                </tr>
              ))}
            </tbody>
