...
import { Loader2, AlertTriangle, Plus, CalendarRange, FileText, Users, ChevronRight } from 'lucide-react'
...
interface CalendarRow {
  ...
  createdAt: string
  _count: { topics: number; accounts: number }
}
...
export default function AdminArticleCalendarsPage() {
  ...
  async function load() {
    ...
    try {
      const [calRes, specRes] = await Promise.all([
        fetch('/api/admin/article-calendars', { cache: 'no-store' }),
      ...
      setCalendars((await calRes.json()).calendars ?? [])
      if (specRes.ok) setSpecs((await specRes.json()).specializations ?? [])
    } catch (err) {
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    setCreateError(null)
    try {
      const res = await fetch('/api/admin/article-calendars', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name, industry, specializationKey: specializationKey || null, hemisphere }),
      })
      if (!res.ok) {
  ...
  return (
    <div className="max-w-4xl">
      <div className="mb-6">
        <h1 className="text-2xl font-semibold text-foreground">Article Calendars</h1>
        <p className="mt-1 text-sm text-muted-foreground">
          Pre-planned article topics scoped to a specialization + hemisphere. Accounts are
          auto-routed to the matching calendar by their primary specialization and country.
        </p>
      </div>
      ...
      {!loading && !error && calendars.length > 0 && (
        <div className="space-y-2">
          {calendars.map((c) => (
            <Link
              key={c.id}
              href={`/admin/article-calendars/${c.id}`}
