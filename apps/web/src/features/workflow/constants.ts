export const STATUS_LABELS: Record<string, { label: string; color: string; bg: string }> = {
  ...
  reviewing:   { label: 'Quality check…',   color: 'text-blue-700 dark:text-blue-300',     bg: 'bg-blue-50 dark:bg-blue-900/40' },
  completed:   { label: 'Needs Approval',   color: 'text-yellow-700 dark:text-yellow-300', bg: 'bg-yellow-50 dark:bg-yellow-900/40' },
  needs_review:{ label: 'Needs review',     color: 'text-amber-700 dark:text-amber-300',   bg: 'bg-amber-50 dark:bg-amber-900/40' },
...
// Statuses where the pipeline is actively running (SSE open + fallback poll).
// 'reviewing' = the automated quality gate is evaluating / rewriting / re-gating.
export const ACTIVE_STATUSES = new Set(['pending', 'in_progress', 'reviewing'])
